...
const LipinskiPage = () => {
  ...
  const handleSubmit = async (data: { smiles: string }) => {
    try {
      setIsLoading(true);
      const result = await MedDrugService.validateLipinski(data.smiles);
      
      setSmiles(data.smiles);
      setResult(result);
      
      if (result.is_valid) {
        toast.success('Molecule passes Lipinski\'s Rule of Five');
      } else {
        toast.warning('Molecule fails Lipinski\'s Rule of Five');
      }
    } catch (error) {
      console.error('Failed to validate molecule:', error);
      toast.error('Failed to validate molecule');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-8">
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold text-pharma-800 mb-2">Lipinski Rule Validator</h1>
        <p className="text-gray-600 max-w-2xl mx-auto">
          Evaluate drug-likeness based on Lipinski's Rule of Five: molecular weight, LogP, 
        ...
        <div className="lg:col-span-2">
          {isLoading ? (
            ...
            <div className="space-y-4">
              ...
              <ResultCard type="lipinski" data={result} />
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center h-64 text-center bg-white bg-opacity-70 rounded-lg border border-gray-200 p-8">
              ...
              <p className="text-gray-500 max-w-md">
                Enter a SMILES string to analyze drug-likeness according to Lipinski's Rule of Five.
              </p>
            </div>
          )}
        </div>
